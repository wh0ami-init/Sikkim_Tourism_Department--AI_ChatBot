...
export default function Admin() {
  ...
  {tab === "overview" && <div className="mt-6 grid gap-x-8 gap-y-1 lg:grid-cols-3"><Stat icon={MapPinned} label="Destinations available" value={dashboard?.destination_count ?? destinations.length} summary={`${dashboard?.destination_count ?? destinations.length} destinations are ready for visitors to explore.`} /><Stat icon={FileUp} label="Official notices" value={circulars.length} summary={`${circulars.length} current notices are available for visitors to check.`} /><Stat icon={Database} label="Travel assistant" value="Ready" summary="Destination and notice information is ready to support visitor questions." /><section className="mt-7 border-y border-border/70 py-7 lg:col-span-3"><h2 className="font-serif text-xl font-bold">Operational actions</h2><p className="mt-1 text-sm text-muted-foreground">Synchronize only after changing the related live data.</p><div className="mt-5 flex flex-wrap gap-3"><ActionButton icon={RefreshCw} label="Sync destinations to AI" onClick={() => sync("destinations")} disabled={loading} /><ActionButton icon={RefreshCw} label="Fetch latest circulars" onClick={() => sync("circulars")} disabled={loading} secondary /></div></section></div>}
